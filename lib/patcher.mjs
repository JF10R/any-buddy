...
export function patchBinary(binaryPath, oldSalt, newSalt) {
  ...
  // Windows: can't rename/delete OVER a running exe, but CAN rename it
  // away — so move the locked binary aside, then move the new one in.
  const stats = statSync(binaryPath);
  const tmpPath = binaryPath + '.anybuddy-tmp';
  const oldPath = binaryPath + '.anybuddy-old';
  ...
  try {
    ...
    if (IS_WIN) {
      // Windows: rename running binary away (allowed), then rename new one in.
      // Clean up any stale .old file from a previous patch.
      try { unlinkSync(oldPath); } catch { /* ignore */ }
      try {
        renameSync(binaryPath, oldPath);  // move locked exe aside
      } catch (mvErr) {
        // If even renaming away fails, we truly can't proceed.
        throw Object.assign(new Error('EPERM'), { code: 'EPERM', cause: mvErr });
      }
      renameSync(tmpPath, binaryPath);    // move patched file into place
      // Try to clean up the old binary (may fail if still locked — that's fine)
      try { unlinkSync(oldPath); } catch { /* ignore, cleaned up on next run */ }
    } else {
      try {
    ...
    if (IS_WIN && (err.code === 'EPERM' || err.code === 'EBUSY')) {
      throw new Error(
